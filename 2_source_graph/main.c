...
#define RX_NODE_NAME1 "rx_node1"
#define RX_NODE_NAME2 "rx_node2"
...
static uint16_t rx_node_process1(struct rte_graph *graph, struct rte_node *node,
                                void **obj, uint16_t nb_objs) {
  uint16_t nb_rx;
//printf("Inside the RX Process Function  with  lcore id :%d\n",rte_lcore_id());
  ...
  if (nb_rx) {
    ...
    printf("Inside the RX_NODE1 Process Node Got %d Packets \n", (int)nb_rx);
...
struct rte_node_register rx_node1 = {.name = RX_NODE_NAME1,
                                    .process = rx_node_process1,
...
rte_node_register_rx_node1(void) {
  rx_node1.parent_id = RTE_NODE_ID_INVALID; /* 0xFFFFFFFF */
  rx_node1.id = __rte_node_register(&rx_node1);
  printf("RX Node1 has been Registered\n");
}


static uint16_t rx_node_process2(struct rte_graph *graph, struct rte_node *node,
                                void **obj, uint16_t nb_objs ) {
  uint16_t nb_rx;
  ...
  if (nb_rx) {
    ...
    printf("Inside the RX_NODE2 Process Node Got %d Packets \n", (int)nb_rx);
    rte_node_enqueue(graph, node, 0, (void **)pkt, nb_rx);
  }
...
struct rte_node_register rx_node2 = {.name = RX_NODE_NAME2,
                                    .process = rx_node_process2,
                                    .flags = RTE_NODE_SOURCE_F,
                                    .nb_edges = 1,
                                    .next_nodes = {
                                        [0] = IPV4_NODE_NAME,
...
static void __attribute__((constructor(65535), used))
rte_node_register_rx_node2(void) {
  rx_node2.parent_id = RTE_NODE_ID_INVALID; /* 0xFFFFFFFF */
  rx_node2.id = __rte_node_register(&rx_node2);
  printf("RX Node2 has been Registered\n");
}
...
int main(int argc, char **argv) {
  ...
  printf("Inside the Main Function  with  lcore id :%d\n",rte_lcore_id());
  ...
  struct rte_graph_param graph_param = {
      .nb_node_patterns = 2,
      .node_patterns = (const char *[]){RX_NODE_NAME1,RX_NODE_NAME2},
  };
